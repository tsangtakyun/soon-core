...
type InboxItem = { id: string; platform: string; source_url: string; source_account?: string; content_text?: string; media_type?: string; thumbnail_url?: string; published_at?: string; industry_codes?: string[] }
...
export function IntelligenceInbox() {
  ...
  const visibleCapturedItems = selectedCandidateIndustry === 'all' ? capturedItems : capturedItems.filter((item) => item.industry_codes?.includes(selectedCandidateIndustry))
  const activeIndustries = industries.filter((industry) => watchlists.some((item) => item.industry_codes?.includes(industry.code)))
  const activeCandidateIndustries = industries.filter((industry) => capturedItems.some((item) => item.industry_codes?.includes(industry.code)))
