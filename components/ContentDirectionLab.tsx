...
export function ContentDirectionLab() {
  ...
  const load = useCallback(async () => {
    ...
    const [directionsResponse, stylesResponse] = await Promise.all([
      ...
      fetch('/api/content-directions/styles', { cache: 'no-store' }),
    ])
  ...
  return <section className="lab"><header><div><small>SOON CONTENT STYLES</small><h1>選擇內容風格</h1><p>以卡片瀏覽內容風格與 reference；最新更新會排最前。</p></div><div className="summary"><strong>{items.length}</strong><span>個風格研究</span></div></header>
    ...
    <style jsx>{`
      .published-styles { margin-bottom: 18px; border: 1px solid #302747; border-radius: 16px; background: #141217; padding: 20px; }
      .published-head { display: flex; justify-content: space-between; gap: 20px; align-items: end; margin-bottom: 15px; }
      .published-head h2 { margin: 5px 0; }
      .published-head p { margin: 0; color: #888; font-size: 11px; }
      ...
      @media (max-width: 1180px) { .style-cards { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
      @media (max-width: 850px) { .cards { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
      @media (max-width: 850px) { .style-cards { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
      @media (max-width: 560px) { .cards { grid-template-columns: 1fr; } }
      @media (max-width: 560px) { .style-cards { grid-template-columns: 1fr; } .published-head { align-items: flex-start; flex-direction: column; } }
    `}</style>
  </section>
}
