...
// A Shot = one sentence / one camera angle to record
type Shot = {
  id: string
  text: string       // content / VO line
  shotType: string   // e.g. "Wide Shot"
  shotDesc: string   // e.g. "交代環境全景"
  recorded: boolean
  skipped: boolean
  videoUrl?: string
}

// A Part = one section of the video (hook, bg, trans, test1-4, ending)
type Part = {
  id: string
  label: string
  type: 'hook' | 'bg' | 'trans' | 'test' | 'end'
  subStep: 'review' | 'camera'  // review content first, then shoot
...
// ─── Camera Component ───
function CameraView({
  onSave, onRetake, onSkip, shotText, shotType, partLabel
}: {
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      try {
        ...
        setCamError('無法開啟鏡頭：' + (e.message || '請允許鏡頭權限'))
      }
    }
  }, [])

  useEffect(() => { startCamera(); return () => { stopStream() } }, [startCamera])
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column' }}>
      {/* Shot info */}
      <div style={{ padding:'10px 20px 10px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
        ...
        <div style={{ textAlign:'right' }}>
          ...
          {state === 'recording' && (
            <div style={{ fontSize:13, color:'#E24B4A', fontVariantNumeric:'tabular-nums' }}>● {fmt(elapsed)}</div>
          )}
        </div>
      </div>
        ...
        {state === 'preview' && (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:28 }}>
            ...
            <button onClick={startRec} style={recBtnStyle}>
              <div style={{ width:44, height:44, borderRadius:'50%', background:'#E24B4A' }} />
            </button>
            <div style={{ width:44 }} />
          </div>
        )}
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'20px 20px 32px', maxWidth:480, width:'100%', margin:'0 auto', gap:16 }}>
      <div>
        ...
        <textarea
          ...
          style={{ width:'100%', background:'var(--bg2)', border:'1px solid var(--border2)', borderRadius:'var(--radius)', padding:'14px 16px', fontSize:14, color:'var(--ink)', lineHeight:1.8, minHeight:120, resize:'vertical', fontFamily:'Inter, sans-serif', outline:'none', boxSizing:'border-box' }}
        />
      </div>
      ...
      <div style={{ display:'flex', gap:10, marginTop:'auto' }}>
        <button onClick={onSkip} style={{ flex:1, background:'none', border:'1px solid var(--border)', borderRadius:'var(--radius-pill)', padding:'13px', fontSize:13, color:'var(--ink3)', cursor:'pointer' }}>跳過呢個 part</button>
...
function ShootInner() {
  ...
  // Split text into shots by sentence
  const splitIntoShots = (text: string, partId: string, baseTypes: string[]): Shot[] => {
    const sentences = text.split(/[。！？，,]/).map(s => s.trim()).filter(s => s.length > 4)
    return sentences.slice(0, 4).map((s, i) => ({
      id: `${partId}-s${i}`,
      text: s,
      shotType: baseTypes[i % baseTypes.length] || 'Medium Shot',
      shotDesc: SHOT_TYPES[baseTypes[i % baseTypes.length]?.[0].toLowerCase() as keyof typeof SHOT_TYPES]?.[0]?.desc || '',
      recorded: false, skipped: false,
    }))
  }
...
請輸出 JSON（唔好加任何其他文字）：
{
  "hookLine": "一句具體 hook，廣東話口語",
  "background": "60-80字廣東話背景介紹，3-4句",
  "transition": "一句轉場旁白，廣東話",
  "tests": [
    { "label": "實測項目", "vo": "試後一句旁白", "price": "價錢或份量" },
    { "label": "實測項目", "vo": "試後一句旁白", "price": "價錢或份量" },
    { "label": "實測項目", "vo": "試後一句旁白", "price": "價錢或份量" },
    { "label": "實測項目", "vo": "試後一句旁白", "price": "價錢或份量" }
  ],
  "ending": "一句收結旁白，廣東話"
      ...
      const hookShot: Shot = { id:'hook-s0', text: parsed.hookLine || hook.teaser, shotType: mode==='self' ? 'Medium Shot' : 'Wide Shot', shotDesc:'主角開場', recorded:false, skipped:false }

      // Background: split into multiple shots
      const bgText = parsed.background || ''
      const bgShots = splitIntoShots(bgText, 'bg', ['Wide Shot','Medium Shot','B-roll'])

      const builtParts: Part[] = [
        {
          id:'hook', label:'Hook', type:'hook', subStep:'review', done:false, skipped:false,
          editableContent: parsed.hookLine || hook.teaser,
          shots: [hookShot],
        },
        {
          id:'bg', label:'背景介紹', type:'bg', subStep:'review', done:false, skipped:false,
          editableContent: bgText,
          shots: bgShots,
        },
        {
          id:'trans', label:'轉場', type:'trans', subStep:'review', done:false, skipped:false,
          editableContent: parsed.transition || trans.name,
          shots: [{ id:'trans-s0', text: parsed.transition || trans.name, shotType:'Medium Shot', shotDesc:'轉場鏡頭', recorded:false, skipped:false }],
        },
        ...(parsed.tests || []).map((t: any, i: number): Part => ({
          ...
          shots: [
            { id:`test${i+1}-s0`, text:t.label + (t.price ? `（${t.price}）` : ''), shotType:'產品特寫', shotDesc:'近鏡拍攝食物質感', recorded:false, skipped:false },
            { id:`test${i+1}-s1`, text:t.vo, shotType:'反應鏡頭', shotDesc:'主持真實反應', recorded:false, skipped:false },
          ],
        })),
        {
          id:'end', label:'Ending', type:'end', subStep:'review', done:false, skipped:false,
          editableContent: parsed.ending || end.name,
          shots: [{ id:'end-s0', text: parsed.ending || end.name, shotType:'Medium Shot', shotDesc:'主角直視鏡頭收結', recorded:false, skipped:false }],
        },
      ]
  ...
  const shotSaved = (url: string) => {
    setParts(prev => prev.map((p, i) => {
      if (i !== partIdx) return p
      const newShots = p.shots.map((s, j) => j === shotIdx ? { ...s, recorded:true, videoUrl:url } : s)
      const allDone  = newShots.every(s => s.recorded || s.skipped)
      return { ...p, shots:newShots, done:allDone }
    }))
    // Next shot or next part
    if (shotIdx < (currentPart?.shots.length || 1) - 1) {
      setShotIdx(i => i + 1)
    } else {
      setParts(prev => prev.map((p, i) => i === partIdx ? { ...p, done:true } : p))
      setPartIdx(i => i + 1)
      setShotIdx(0)
    }
  }
  ...
  const shotSkipped = () => {
    setParts(prev => prev.map((p, i) => {
      if (i !== partIdx) return p
      const newShots = p.shots.map((s, j) => j === shotIdx ? { ...s, skipped:true } : s)
      const allDone  = newShots.every(s => s.recorded || s.skipped)
      return { ...p, shots:newShots, done:allDone }
    }))
    if (shotIdx < (currentPart?.shots.length || 1) - 1) {
  ...
  // ── Finished Screen ──
  const isFinished = partIdx >= totalParts
  if (isFinished && step === 'shoot') return (
    <main style={{ minHeight:'100dvh', background:'var(--bg)', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:20, padding:32 }}>
        ...
        <p style={{ fontSize:14, color:'var(--ink3)', textAlign:'center', lineHeight:1.8 }}>
          拍咗 {parts.filter(p=>p.done).length} 個 part<br/>
          跳過 {parts.filter(p=>p.skipped).length} 個 part<br/>
          片段已 save 喺相簿
        </p>
    ...
    <main style={{ minHeight:'100dvh', background:'var(--bg)', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:'10px 20px 8px', borderBottom:'1px solid var(--border)' }}>
        <div style={{ display:'flex', gap:3, marginBottom:5 }}>
          {parts.map((p, i) => (
            <div key={p.id} style={{ flex:1, height:3, borderRadius:2, background: p.done ? 'var(--green)' : p.skipped ? 'var(--border2)' : i===partIdx ? 'var(--accent)' : 'var(--border)', transition:'background 0.3s' }} />
          ))}
        </div>
        <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'var(--ink3)' }}>
          ...
          <span style={{ color:'var(--accent2)' }}>
            {currentPart?.label}
            {currentPart?.subStep === 'camera' && currentPart.shots.length > 1 && ` · 鏡頭 ${shotIdx+1}/${currentPart.shots.length}`}
          </span>
        </div>
      </div>
