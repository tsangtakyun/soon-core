import type { IGBlockType, IGSegmentType } from '@/lib/ig-script'

export const runtime = 'nodejs'
export const maxDuration = 60

type ReviewBlock = {
  type: IGBlockType | 'other'
  typeLabel: string
  speaker: string
  content: string
}
...
${block.content}`
  ...
  const userPrompt = `請審閱以下 IG 腳本段落：

段落類型：${body.segmentTypeLabel}
段落標題：${body.segmentTitle || '(未命名)'}
建議時長：${body.suggestedTime || '(未填)'}
內容：
${blockText || '(沒有內容)'}
...
請以 JSON 格式回應，不要 markdown，不要額外文字：
{
  ...
  "score": 8,
  "blocks": [
    { "block_index": 0, "type": "...", "issue": "...或null", "suggestion": "...或null" }
  ],
  "clarity": "清晰度",
  "typos": "錯字或null",
  "timing": "時長是否合適的評語"
  ...
  const upstream = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1800,
      system: '你係一個專業 IG 短片內容顧問，專門審閱廣東話短片腳本。你必須只返回 JSON。',
