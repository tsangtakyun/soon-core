...
async function runCollection(watchlistId?: string) {
  ...
  for (const row of (data ?? []) as WatchlistRow[]) {
    try {
      ...
      const records = items.map((item) => ({ workspace_id: row.workspace_id, watchlist_id: row.id, platform: row.platform, platform_item_id: item.platformItemId, source_url: item.sourceUrl, source_account: item.sourceAccount || null, content_text: item.contentText || null, media_type: item.mediaType || null, thumbnail_url: item.thumbnailUrl || null, media_children: item.mediaChildren, published_at: item.publishedAt || null, source_scope: row.consent_basis === 'public_research' ? 'public_research' : 'workspace_private', industry_codes: row.industry_codes ?? [], raw_payload: item.rawPayload, content_hash: captureHash(item) }))
      if (records.length) {
